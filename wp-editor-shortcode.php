...
function p_init(){
	...
	add_menu_page('Editor_shortcode', "Editor shortcode", "manage_options", "editorshortcodes", "AJ_editorshortcodes",plugins_url('ico.png',__FILE__),"10");

add_action( 'admin_init', 'AJ_update_editorshortcodes' );
}

if( !function_exists("AJ_update_editorshortcodes") )
{
function AJ_update_editorshortcodes() {
...
function AJ_slider_shortcode($atts) {
$html="";
$atts = shortcode_atts(
		array(
			'img' => '',
...
add_shortcode('slider-shortcode', 'AJ_slider_shortcode');

function AJ_custom_scripts_method() {
wp_enqueue_script('slick-js', plugins_url().'/wp-editor-shortcode/assets/js/slick.min.js');
wp_enqueue_style('slick-css', plugins_url().'/wp-editor-shortcode/assets/css/slick.css');
//wp_enqueue_script('custom-editorjs', plugins_url().'/custom-plugin/assets/js/custom.js');
}
add_action( 'wp_footer', 'AJ_custom_scripts_method' );


function AJ_upload_mce_button() {
  ...
  if ( 'true' == get_user_option( 'rich_editing' ) ) {
    add_filter( 'mce_external_plugins', 'AJ_upload_button_tinymce_plugin' );
    add_filter( 'mce_buttons', 'AJ_upload_button_register_mce_button' );
  }
}
add_action('admin_head', 'AJ_upload_mce_button');

// Function for new button
function AJ_upload_button_tinymce_plugin( $plugin_array ) {
  $plugin_array['upload_mce_button'] = plugins_url() .'/wp-editor-shortcode/assets/js/custom-editor.js';
  return $plugin_array;
}

// Register new button in the editor
function AJ_upload_button_register_mce_button( $buttons ) {
  array_push( $buttons, 'upload_mce_button' );
  return $buttons;
}

add_action( 'wp_footer', 'AJ_slider_custom_script', 100 );
function AJ_slider_custom_script(){
